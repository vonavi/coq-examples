...
:Link: https://stackoverflow.com/q/68056978
|*)
...
hierarchy e.g. ``Set = Type(0) <= Type = Type(1) <= Type(2) <= ...``.
I guess this sort of makes sense intuitively like I'd assume ``nat \in
Type`` and all usual programming types to be in it but not sure what
would be in ``Type`` that wouldn't be in ``Set``. Perhaps recursive
...
``Set`` being different from ``Type``. In particular, Adam's comment
would also make sense if you replace ``Set`` by ``Type`` everywhere.
...
in ``Set`` or ``Type`` *rather than* ``Prop``. This is because things
that live in ``Prop`` are erased when you extract programs from Coq,
so something defined in ``Prop`` would end up not computing anything.

As for your second question: ``Set`` is something that lives in
``Type``, but not in ``Set``, as the following snippet shows.
|*)
...
is pretty much the only difference you see between ``Set`` and
``Type`` by default. You can also make them more different by invoking
