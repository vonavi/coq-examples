...
(*|
Note that you need the `proj1_sig` function to extract `n` from the
subset type `{n : nat | Nat.even n = true}`. To use `add_two_even`,
you also need to go the other way around: go from a number to an
element of `{n | Nat.even n = true}`. In Coq, this requires manual
proof. For concrete values of `n`, this is easy:
|*)
...
concrete number, Coq can evaluate `Nat.even n` and find if `eq_refl`
is a valid proof of `Nat.even n = true`. When `n` is `1`, `Nat.even n
= false`, and the check fails, leading to the first error message.
When `n` is `4`, the check succeeds.
...
know that `Nat.even (n + n) = true` for any value of `n`, but Coq does
not. Thus, to call `add_two_even` on something of the form `n + n`, we
need to show a lemma.
|*)
...
example, consider the `eq_from_tnth` lemma, which says that two lists
