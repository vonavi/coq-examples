...
https://stackoverflow.com/questions/66064683/proofs-of-structural-properties-of-arguments-in-match-in-coq
|*)
...
(*|
In words, we first define a version of `zip` that does not care about
lengths, and then we use that to define the function you're looking
for.
...
read. In particular, note how `zip'` has a match returning a function.
This idiom, known as the *convoy pattern*, is needed whenever we need
to refine the type of an argument in a pattern matching branch.
Actually, `zip'` is even worse than what you might think, because the
tactics that discharge the proof obligations generate code. Try
printing `zip'` to see what the definition really looks like! Sadly,
this ugliness is not just cosmetic: these more complicated definitions
are much harder to reason about. For instance, it is possible to prove
that `zip` and `zip'` always produce the same outputs. Try it to see
how fun it is!
...
of the day they will still generate code that is equivalent to `zip'`.
And in this case, the length hypothesis doesn't buy us much.
...
lemma, or we can work with length-indexed lists. Here is one possible
definition:
|*)
...
(*|
The `{.. | ..}` is Coq's notation for refinement, or subset types. We
can then repackage some of the functions over lists to work over
`vec`. For instance, we can show that `map` takes `vec A n` to `vec B
n`. This approach pays off if you don't use many functions that
require you to change the length index `n`, because in those cases you
need to reason about the equality of complicated length expressions on
types, which Coq is not very good at. For `vec` in particular, I would
recommend you to have a look at the `tuple` library of mathcomp
(available `here
