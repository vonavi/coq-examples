...
`t1; t2` is not the same as doing first `t1` then `t2`. If you want to
do this you can do `t1. t2.` and this is the way to step through them.
...
- and it allows backtracking, if `t2` were to fail, it would try a
  different *success* for `t1` and apply `t2` again on the generated
  subgoals;
...
closed by applying an introduction rule (`constructor`) and that then
it should be easy. If multiple intro-rules/constructors apply doing
