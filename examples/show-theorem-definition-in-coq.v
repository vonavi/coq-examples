...
https://stackoverflow.com/questions/62939436/show-theorem-definition-in-coq
|*)
...
function to a certain number arguments, which `apply` infers or leaves
to the user as goals. The tactics `induction` and `rewrite` are
similar, but they apply theorems that are not given by the user. The
tactic `destruct` essentially produces a piece of programs that is a
...
directory `...theories/Reals`. The fourth name should not be used as a
directory name, but as file name. So you should look in the file
`RIneq.v`
