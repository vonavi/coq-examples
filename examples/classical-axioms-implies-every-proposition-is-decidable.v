...
(*|
Yet, with what I know of Coq, I cannot implement `decide` as I cannot
destruct `(LEM p)` (of sort `Prop`) to return something other than a
`Prop`.
...
`or_introl`) or a proof that `B` holds (using the other constructor).
So a proof of `A \/ ~ A` is either a proof that `A` holds or a proof
that `~ A` holds.

Following this meta-theoretical property, in **Coq without any
axioms**, all proofs of propositions of the form `forall x, P x \/ ~P
x` actually are proofs that `P` is decidable. In this paragraph, the
meaning of `decidable` is the commonly accepted meaning, as used by
computability books.

Some users started using the word `decidable` for any predicate `P` so
that there exists a proof of `forall x, P x \/ ~ P x`. But they are
actually talking about a different thing. To make it clearer, I will
call this notion `abuse-of-terminology-decidable`.
...
course, you cannot change the meaning of `conventionally-decidable` by
just adding an axiom in you Coq development, so there is no inclusion
anymore.
...
Proofs of such propositions are often named with `_dec` suffix, where
`_dec` directly refers to *decidable*. This abuse is less strong, but
it is still an abuse of terminology.
