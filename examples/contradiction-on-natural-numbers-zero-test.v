...
`Prop` to your context, making it easy to then use `lia`, `omega`,
etc. to finish the proof.
...
(*|
It will create two branches, in one `n ?= 0` is replaced with `true`
and `n = 0` is added to the context, and in the other `n =? 0` is
replaced with `false` and `n <> 0` is added to the context. It is now
very easy to finish of the proof, as the first goal's context contains
the contradiction `n = 0` and `n <> 0`, and the second goal is `false
= false`. You can use the automation tactical `now`, so the complete
proof is
|*)
...
Generally, you can use the fact that `=?` reflects equality on natural
numbers (`Nat.eqb_spec`). Are you using the two notions of equality on
purpose? Note that `n <> 0` is a notation for `n = 0 -> False`.
|*)
