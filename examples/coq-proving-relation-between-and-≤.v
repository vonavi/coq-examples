...
to do the induction on that, rather than `n`. `le` doesn't make any
references to `0` or even `S n` (it does have `S m`), so induction on
`n` is probably not the way to go. Induction on `m` *might* work, but
it's probably harder than necessary.
...
though). If you assume that `n <= m`, then by the inductive definition
of `lt`, this means that either `n` and `m` are the same, or `m` is
the successor of some number `m'` and `n` is less than or equal to
`m'` (can you see why the definition of `lt` implies this?). In the
first case, we'll have to use the additional hypothesis that `n <> m`
to derive a contradiction. In the second case, we won't even need
that. `n <= m'` implies that `S n <= S m'`, so since `m = S m'`, `S n
<= m`, i.e., `n < m`.
...
prove everything for every `n`. With `le'_ind`, you only need to prove
it for the particular `n` that you're using. This can sometimes
simplify proofs, though it's not necessary for the proof of your
theorem. It's a good exercise to prove that these two predicates are
equivalent.
|*)
