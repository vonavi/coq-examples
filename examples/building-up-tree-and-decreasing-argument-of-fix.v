...
obtained via some arbitrary function (in this case, `div` and `sub` in
`copy2 a ((i - 1) / 2)`).
...
(*|
There is also a conversion function `N.of_nat : nat -> N`, although it
might also be a good idea to use `N` everywhere instead of `nat`, if
the conversions become too annoying.

The final definition starts by case analysis on `N`, and the case
revealing a `positive` number is handled with a `fix`-point, where the
base case is `1` instead of `0`. We have to shift some details,
because the even case is `2p` instead of `2p + 2`, so instead of a
pair of trees of size `(i+1, i)` we have to do `(i-1, i)`. But overall
the recursive cases still naturally match an informal specification:
|*)
...
decreasing argument), the argument `n` can only decrease by a constant
at most (constant with respect to `n`), since there are only finitely
many `match` in a definition. In particular, there is no way to make a
recursive call on the result of a division by two, as that represents
a decrease by `n/2`, a value linear in `n`.
...
And indeed, the division of a Peano `nat` can be defined in such a way
that Coq can tell that the result is a subterm of the dividend:
|*)
...
(*|
The idea is to write a `fix`-point of two arguments (somewhat like the
fuel solution), which start out equal (`d2 n n`), and we remove
**two** `S` constructors from one (`n2`) of them for every **one** `S`
we remove from the other (`n1`). Important details:

- In all the non-recursing cases, we return `n1` (and *not* `0` in any
  case), which is then guaranteed to be a subterm of the topmost `n`.
...
Once we have `div2`, we can define `copy` with a few tweaks to obtain
`i-1` and `i-2` as *proper subterms* of `i`, again by
pattern-matching. Below, `i'` and `i''` are proper subterms of `i` (by
visual inspection), and `div2 i'` and `div2 i''` are subterms of `i'`
and `i''` (by the definition of `div2`). By transitivity they are
proper subterms of `i`, so the termination checker accepts.
|*)
