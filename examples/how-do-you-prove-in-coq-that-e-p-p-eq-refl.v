...
(*|
But there just seems to be no way to do it. The problem is the type `p
= p` being an equality on the same term, and then trying to match its
instance. If this were not the case, it is easy enough to prove that a
term of a type with a single constructor is equal to that constructor.
|*)
...
(*|
The problem is that the `return` clause here translates internally to
a predicate function something like this:
...
the identity type `x = y` is the set of morphisms from `x` to `y` in
the groupoid. In this model, there can be more than one distinct `e: x
= y`.
