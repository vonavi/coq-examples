...
**EDIT** I added the `Hint Unfold f.`, and increased bound to 100 but
I can't see any unfolding done with `debug auto 100`:
|*)
...
The tactic `auto` is meant to find a proof if a manual proof only uses
`apply` with a restricted set of theorems. Here the restrict set of
theorems is taken from the `core` hint database. For the sake of
brevity, let's assume this database only contains `le_S`, `le_n`,
`plus_n_O` and `plus_n_Sm`.

To simplify, let's assume that we work with the goal `a <= 2 ^ a`. The
head predicate of this statement is `_ <= _` so the tactic will only
look at theorems whose principal statement is expressed with `_ <= _`.
This rules out `plus_n_O` and `plus_n_Sm`. Your initiative of adding
these goes down the drain.

If we look at `le_n` the statement is `forall n : nat, n <= n`. If we
replace the universal quantification by a *pattern variable*, this
pattern is `?1 < ?1`. Does this unify with `a <= 2 ^ a`? The answer is
no. so this theorem won't be used by `auto`. Now if we look at `le_S`,
the pattern of the principal statement is `?1 <= S ?2`. Unifying this
pattern with `a <= 2 ^ a`, this would require `?1` to be `a`. Now what
could be the value of `?2`? We need to compare symbolically the
expressions `2 ^ a` and `S ?2`. On the left hand side the function
symbol is either `_ ^ _` or `2 ^ _` depending on how you wish to look
at it, but anyway this is not `S`. So `auto` recognizes these
functions are not the same, the lemma cannot apply. `auto` fails.
...
(*|
Then `Hint Unfold myle : core.` would be useful to make sure that the
database theorems for `_ <= _` are also used to proved instances of
`myle`, as in the following example (it fails if we have 4 occurrences
of `S`, because of the depth limitation).
|*)
...
(according to the definition of addition) but not provable by `auto`.
Even if you add `Hint Unfold plus : core.` as a directive, this won't
help because `plus` is not the head symbol of the goal.
|*)
...
symbol" of the current goal makes any progress, `auto` will fail. And
if `auto` reaches maximum depth, it will fail.

Note that `auto` it does not automatically apply the `unfold` tactic.
There's no way to solve `a <= f(a)` when `f` is opaque, without
further assumptions. If you want, you can use `Hint Unfold f
