...
value is used? Is it correct to say that the return value of `ex1` is
an instance of the type `forall n : nat, 7 * 5 < n -> 6 * 6 <= n`?
|*)
...
`ex1`, or that `ex1` has type `forall n, 7 * 5 < n -> 6 * 6 <= n`.
...
(*|
Simplifying this proof means replacing `H` by `fun H' : True => H'`
and reducing the application, which yields `I`. You can see this by
asking Coq to compute this term:
|*)
...
result, `H`, was given type `7 * 5 < n`. But `a < b` is defined as `S
a <= b`; thus we can also view `H` as having type `S (7 * 5) <= n`.
Coq now needs to ensure that `H` has type `6 * 6 <= n`, which it does,
because the two lower bounds compute to 36. Thus, there is computation
happening when you enter a proof in Coq, but the computation is
performed by the type-checker, not the proof term (even though the
proof term does have computational behavior).
|*)
