...
(*|
What puzzled me is the `destruct H` in the second case. What does the
tactic `destruct` do here? It seems different from What I've known
about it before. (`H` here is `~ (exists x : X, ~ P x)`). After using
`destruct H`, the subgoal is tranformed from `P x` into `exists x : X,
~ P x`.
|*)
...
When you destruct a term of the form `A -> B` you get a goal for `A`
and the goals for what `destruct B` would result in. `not A` is
defined as `A -> False` so `B` is `False` in your case and `destruct
B` results in no goals. So you end up with just `A`.
