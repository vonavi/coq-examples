...
:Link: https://stackoverflow.com/q/69710435
|*)
...
is. I know that ``x = y \/ x <> y`` in ``Prop`` and ``{x = y} + {x <>
y}`` is in ``Set``, and I can prove the first one from the second one
but not the other way around. As far as I understand it, it is because
I am not allowed to construct values of type ``Prop`` from values of
type ``Set``.
...
Intuitively, we interpret objects in ``Set`` as programs, and objects
in ``Prop`` as proofs. So the decidable equality type is the type of a
...
that any two elements of ``A`` are either equal of unequal. Notably,
we would not be able to inspect *which* outcome is the case for
specific values of ``x`` and ``y``, at least outside of case analysis
within a proof. This is the result of the ``Prop`` elimination
restriction that you alluded to (although you got it backwards: one is
not allowed to construct values of sort ``Set``/``Type`` by
eliminating/matching on an element of sort ``Prop``).
...
a ``Prop`` (for example, if we axiomatized ``forall P, {P} + {~P}``),
then we would likely not accept the axiom, since it would instead be
