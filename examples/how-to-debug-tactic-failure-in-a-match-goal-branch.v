...
Take as an example this fake tactic where ``apply A, B, C`` has
several chances for something to go wrong. I've been fighting with a
real tactic somewhat similar to this today.
|*)
...
(*|
it will first try to find some ``n : nat`` in your hypotheses,
starting from the most recent, and then try the tactic ``destruct n;
reflexivity``. If it fails, it will try to find another natural
number. If all of them fail, then it will look if the goal matches the
second clause instead, and if so execute ``exact 0``. If that fails
again, it will backtrack once again and conclude that ``No matching
clauses for match goal``.
...
it fails, no backtracking in the ``lazymatch``, and it will give you
the error of the tactic in the corresponding branch.
...
In case, ``lazymatch`` is not semantically equivalent to your match
and backtracking needs to be involved then it will be harder, but
