...
heterogeneous lists, I wanted to define an equivalent of the `Forall`
function on normal lists. This isn't too difficult, and you end up
with two constructors as usual. Now suppose that I know that a fact is
true about every element of a non-empty list. With normal lists, I
could use `Forall_inv` and `Forall_inv_tail` to assert that it's true
about the head and tail of the list.
...
Alternatively, consider defining `hForall` as a `Fixpoint`. Then the
inversion lemma follows by just unfolding the definition.
|*)
...
CPDT, but briefly the issue is that when we do `destruct`/`inversion`
on `hForall (HCons x hl)`, the index `HCons x hl` gets generalized
before the case-split, so you get a nonsensical case where it is
replaced with `HNil`, and a second case with a *different* index
`HCons x0 hl0`, and a good way of remembering the (heterogeneous)
equality across that generalization is a research-grade problem. You
wouldn't need to mess with heterogeneous equalities if the goal just
got rewritten with those variables, and indeed you can refactor the
goal so that it explicitly depends on `HCons x hl`, instead of `x` and
`hl` separately, which will then be generalized by `destruct`:
|*)
