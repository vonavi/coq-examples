...
expression in `H2` satisfies `eq_refl` constructor and therefore `eq`
proposition is `True`. Then it simplifies the expression:
...
(*|
This one was more complex, we had to introduce a new hypothesis `Heq`.
But we got what we need - `H3` at the end.
...
`specialize`, in its simplest form, simply replaces a given hypothesis
with that hypothesis applied to some other term.
...
(*|
we initially have the hypothesis `H: A -> B`. When we call `specialize
(H a)`, we apply `H` to `a` (apply as in function application). This
gives us something of type `B`. `specialize` then gets rid of the old
`H` for us and replaces it with the result of the application. It
gives the new hypothesis the same name: `H`.

In your case, we have `H2: 1 = 1 -> False`, which is a function from
the type `1 = 1` to the type `False`. That means that `H2` applied to
`eq_refl` is of type `False`, i.e. `H2 eq_refl: False`. When we use
the tactic `specialize (H2 eq_refl).`, the old `H2` is cleared and
replaced by a new term (`H2 eq_refl`) whose type is `False`. It keeps
the old name `H2`, though.

`specialize` is useful when you're sure that you're only going to use
...
`specialize` is a core tactic defined directly in the ltac plugin. It
doesn't use any other tactic internally, since it is its internals.

If you want to keep a hypothesis, you can use the `as` modifier, which
works for both `specialize` and `apply`. In the proof
|*)
...
(*|
if you do `specialize (H a) as H0.`, instead of clearing `H`, it'll
introduce a new hypothesis `H0: B`. `apply H in a as H0.` has the same
effect.
|*)
