...
https://stackoverflow.com/questions/66179133/removing-trivial-match-clause-in-coq
|*)
...
If you restrict `castLemma0` to the `eq_refl` case, that is `forall (x
: A), cast A eq_refl x = x`, this is provable by reflexivity.

One way to understand why this is not provable, is to accept that it
is consistant to assume an axiom `bool_eq_not : bool = bool` such that
`cast bool bool_eq_not x = not x`. Plugging in `bool_eq_not` for `prf`
in `castLemma0` would imply that `not x = x`, which is certainly
wrong.
