...
(*|
Note the `WW` constructor – it can only be value of types `Wrapped A
X` and `Wrapped B Y`.
...
(*|
Why does it happen? `Wrapper` forces contained `Wrapped` to be `A`
version, the type signature forces `Y` and `WW` constructor forbids
being `A` and `Y` simultaneously. I don't understand why this case is
...
This made the first argument to `MyTy` an index of `MyTy`, as opposed
to a parameter. An inductive type with a parameter may look like this:
|*)
...
(*|
Parameters are named on the left of the `:`, and are not `forall`-d in
the definition of each constructor. (They are still present in the
constructors' types outside of the definition: `cons : forall (A :
Type), A -> list A -> list A`.) If I make the `Set` a parameter of
`MyTy`, then `extract` can be defined:
|*)
...
(Or, rather, the underlying `match` expression in Gallina ignores the
indices. When you write a `match` in the source code, Coq tries to
convert it into the primitive form while incorporating information
from the indices, but it often fails.) The fact that `m : MyTy nat` in
the first version of `extract` simply did not matter. Instead, the
match gave me `S : Set` (the name was automatically chosen by Coq) and
`x : S`, as per the constructor `MkMyTy`, with no mention of `nat`.
Meanwhile, because `MyTy` has a parameter in the second version, I
actually get `x : nat`. The `_` is really a placeholder this time; it
is mandatory to write it as _, because there's nothing to match, and
you can `Set Asymmetric Patterns` to make it disappear.
...
argument to `Wrapper` is an index, so the fact that `wr : Wrapper Y`
is ignored; you would need to handle the `Wrap X w` case too. Coq
...
`match`-elaboration to use information from `Wrapped`'s index. If you
issue `Print test.`, you can see that there's a bit of hoop-jumping to
pass information about the index `Y` through the primitive `match`'s
which would otherwise ignore it. `See the reference manual for more
information.
...
In this solution I changed `match` to return a function that takes two
proofs and brings them to the context of our actual result:
